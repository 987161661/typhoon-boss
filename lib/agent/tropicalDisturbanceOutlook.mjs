...
export function buildTropicalDisturbanceOutlook(inputs, options = {}) {
  ...
  const evidence = assessTropicalDisturbanceOutlook(outlook);
  return {
    ...outlook,
    evidenceStatus: evidence.status,
    coverage: evidence.coverage,
    summary: buildEvidenceAwareSummary(outlook, evidence)
  };
}

/**
 * The single seam for interpreting source health. An empty parsed collection is
 * evidence of "none reported" only when its source coverage is available.
 */
export function assessTropicalDisturbanceOutlook(outlook) {
  const coverage = {
    nearTerm: sourceCoverage(outlook?.sources?.jtwc),
    week2: sourceCoverage(outlook?.sources?.cpcWeek2),
    week3: sourceCoverage(outlook?.sources?.cpcWeek3)
  };
  const states = Object.values(coverage);
  const status = states.every((value) => value === "unavailable")
    ? "unavailable"
    : states.every((value) => value === "available")
      ? "available"
      : "degraded";
...
function sourceCoverage(source) {
  ...
  return source.status === "fresh" ? "available" : "degraded";
}

function buildEvidenceAwareSummary(outlook, evidence) {
  const nearTerm = evidence.coverage.nearTerm === "available"
    ? outlook.nearTermDisturbances.length
      ? `JTWC 在西北太平洋列出 ${outlook.nearTermDisturbances.length} 个热带扰动`
      : "JTWC 当前未列出热带扰动"
    : evidence.coverage.nearTerm === "unavailable"
      ? "JTWC 资料不可用，近24小时扰动状态未知"
      : `JTWC 资料时效待核验，已解析 ${outlook.nearTermDisturbances.length} 个扰动`;
  const extendedCoverage = [evidence.coverage.week2, evidence.coverage.week3];
  const extended = extendedCoverage.every((value) => value === "available")
    ? `延伸期有 ${outlook.extendedRangeAreas.length} 个 CPC 概率区域`
    : extendedCoverage.every((value) => value === "unavailable")
      ? "CPC 第2周和第3周资料均不可用，延伸期状态未知"
      : `CPC 延伸期资料不完整，已取得资料中解析到 ${outlook.extendedRangeAreas.length} 个概率区域`;
  return `${nearTerm}；${extended}。`;
}

export function renderTropicalDisturbanceReport(outlook) {
  const evidence = assessTropicalDisturbanceOutlook(outlook);
  const nearTermItems = outlook.nearTermDisturbances.length
    ...
    : "";
  const nearTerm = evidence.coverage.nearTerm === "unavailable"
    ? "- JTWC 西北太平洋显著天气公报本轮未取得，近24小时扰动状态未知；不得把资料缺失解释为没有胚胎候选。"
    : evidence.coverage.nearTerm === "degraded"
      ? `${nearTermItems ? `${nearTermItems}\n` : ""}- JTWC 公报时次过期或待核验，以上仅保留为延迟资料，不能代表当前完整扰动状态。`
      : nearTermItems || "- JTWC 最新西北太平洋显著天气公报在未来24小时监测窗内未列出热带扰动；这表示当前没有官方近时胚胎候选，不表示生成概率为零。";

  const extendedItems = outlook.extendedRangeAreas.length
    ...
    : "";
  const extendedCoverage = [evidence.coverage.week2, evidence.coverage.week3];
  const extended = extendedCoverage.every((value) => value === "unavailable")
    ? "- NOAA CPC 第2周和第3周概率产品本轮均未取得，延伸期生成状态未知；不得报告为 0 个概率区域。"
    : extendedCoverage.every((value) => value === "available")
      ? extendedItems || "- NOAA CPC 最新第2周和第3周产品均已取得，未解析到落入西北太平洋的热带气旋生成概率区。"
      : `${extendedItems ? `${extendedItems}\n` : ""}- NOAA CPC 延伸期资料覆盖不完整或时次待核验；以上只代表已取得产品，不能据此排除其他概率区域。`;
