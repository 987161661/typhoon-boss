...
import { getMarineLayer, noStoreHeaders, type WindFieldBounds } from "@/lib/environmentData";
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await getMarineLayer(bounds), { headers: noStoreHeaders() });
  } catch (error) {
    return NextResponse.json({ source: "Open-Meteo Marine API", status: "unavailable", warnings: [error instanceof Error ? error.message : String(error)] }, { status: 503, headers: { ...noStoreHeaders(), "Retry-After": "120" } });
  }
}
