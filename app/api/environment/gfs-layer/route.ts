...
export async function GET(request: NextRequest) {
  ...
  if (!isGfsScalarLayerId(layer)) {
    return NextResponse.json({ error: "Unsupported GFS layer." }, { status: 400, headers: noStoreHeaders() });
  }
  const bounds = parseViewportBounds(request.nextUrl.searchParams);
  if (!bounds) {
    return NextResponse.json({ error: "A finite west/south/east/north viewport is required." }, { status: 400, headers: noStoreHeaders() });
  }
  try {
    const payload = await getGfsScalarLayer(layer, bounds);
    ...
    }, { status: 503, headers: { ...noStoreHeaders(), "Retry-After": "120" } });
  }
}
...
function parseViewportBounds(params: URLSearchParams): WindFieldBounds | null {
  ...
  if (values.east <= values.west || values.north <= values.south) return null;
  return values;
}
